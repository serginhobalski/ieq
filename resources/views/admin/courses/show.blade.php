...
<div class="container">
    ...
    <div class="card mb-4 border-0 shadow-sm bg-primary text-white">
        <div class="card-body d-flex align-items-center">
            <img src="{{ $course->cover_url }}" class="rounded me-3 border border-white" width="80" height="80" style="object-fit: cover">
            <div>
            ...
            <div class="ms-auto">
                <a href="{{ route('admin.courses.edit', $course) }}" class="btn btn-outline-light btn-sm">Editar Detalhes</a>
            </div>
        </div>
    </div>
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            <span class="badge bg-light text-dark me-3 border rounded-pill">{{ $lesson->order }}</span>
                            <div>
                                <div class="fw-bold">{{ $lesson->title }}</div>
                                @if($lesson->video_url)
                                    <small class="text-muted"><i class="bi bi-camera-video"></i> Vídeo incluído</small>
                                @endif
                            </div>
                        </div>
                        <div class="btn-group">
                            <form action="{{ route('admin.lessons.destroy', $lesson) }}" method="POST" onsubmit="return confirm('Apagar aula?')">
                                @csrf @method('DELETE')
                ...
                <div class="card-body">
                    <form action="{{ route('admin.courses.lessons.store', $course) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" placeholder="Ex: Introdução ao Gênesis" required>
                        </div>
                        <div class="mb-3">
